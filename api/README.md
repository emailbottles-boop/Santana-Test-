...
`wrangler.toml` lists `mahoganyjr.com`, `www.mahoganyjr.com`,
`santanaturner.com` and `www.santanaturner.com` as **Workers Custom
Domains**. When `deploy.sh` runs, Cloudflare creates the DNS records and
the HTTPS certificate for each one by itself. You never open a DNS panel.

The one thing it needs is for each domain to be a zone in *your* Cloudflare
account (both of his are treated the same way):
